...
function timeSpanToString(startDate, endDate) {
  const innerData = new Date(+endDate - +startDate);
  return `${innerData.getUTCHours().toString().padStart(2, '0')}:${innerData
    .getMinutes()
    .toString()
    .padStart(2, '0')}:${innerData
    .getSeconds()
    .toString()
    .padStart(2, '0')}.${innerData
    .getMilliseconds()
...
function angleBetweenClockHands(date) {
  ...
  let angle = 0.5 * (60 * hour - 11 * date.getMinutes());
